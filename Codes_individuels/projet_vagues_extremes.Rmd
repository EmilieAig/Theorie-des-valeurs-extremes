...
# Préparation des données avec catégorie de station (intéret ou autre)
...
quantile_conditionnel <- function(donnees_Y_brutes, maxima_Y, maxima_X, 
  ...
  resultats <- data.frame(p = p_vals, z_p = NA, periode_retour = c("100 ans", "500 ans", "1000 ans"))
...
# On passe les données horaires brutes de SA pour estimer P(Y > 4)

cat("\n=== Paire SA-SB (413 km) ===\n")
res_SB <- quantile_conditionnel(donneesVague$station1, maxima_SA, maxima_SB,
                                 gevSA, gevSB, modlog, y = 4)
print(res_SB)
